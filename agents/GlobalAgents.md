...
Para comandos de instalação e configuração de ESLint, Prettier, commitlint e Husky, consulte o guia rápido em [`settings/commands.md`](../settings/commands.md).

Os arquivos de configuração padronizados (ESLint, Prettier, commitlint, EditorConfig, VS Code) estão disponíveis em [`settings/`](../settings/). Consulte também [`settings/configuracoes-ferramentas.md`](../settings/configuracoes-ferramentas.md) para entender o padrão de centralização adotado.
